...
export default function MPTEfficientFrontier({ allocations, covarianceMatrix, industrialRiskScale }) {
  const [chartView, setChartView] = useState('all'); // 'all' | 'frontier' | 'matrix'
  ...
  // Expected returns for urban asset classes:
  // Residential: 6.8%, Commercial: 8.2%, Industrial: 9.5%
  ...
  // Generate Feasible Portfolio Universe and Efficient Frontier Curve
  const { feasiblePoints, frontierPoints, singleAssets, currentPoint } = useMemo(() => {
    const points = [];
    const step = 0.04; // 25 intervals across simplex

    // Sample simplex w1 + w2 + w3 = 1
    ...
    // Extract Efficient Frontier: for each return level, find the minimum volatility
    // Sort points by return
    const sortedByRet = [...points].sort((a, b) => a.ret - b.ret);
    const frontier = [];
    let minVolSeen = Infinity;

    // From highest return down to lowest, record points on upper-left envelope
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const isSingle = data.name && data.name.startsWith("100%");
  ...
  return (
    <div id="mpt-efficient-frontier-card" className="bg-[#1e1e2e] border border-[#313244] rounded-xl p-5 shadow-lg space-y-4">
      {/* Header */}
          ...
          <div className="w-full h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ScatterChart margin={{ top: 15, right: 20, bottom: 20, left: 0 }}>
                ...
                {/* Risk-Free Benchmark Reference Line */}
                <ReferenceLine
                  y={riskFreeRate}
                  stroke="#f38ba8"
                  strokeDasharray="4 4"
                ...
                {/* 1. Feasible Portfolio Universe (Background Cloud) */}
                <Scatter
                ...
                {/* 2. Efficient Frontier Curve (Optimal Envelope) */}
                <Scatter
                ...
                {/* 4. Active Portfolio Target w* (Glowing Foreground Marker) */}
                <Scatter
